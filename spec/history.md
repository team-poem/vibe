...
## 2026-07-05 (feat/i18n)

### 목적

사용자 요청으로 UI 다국어 지원 (영어/한국어 2종). 설정에서 전환 가능해야 하고
웹뷰 UI 와 트레이 메뉴 모두 적용.

### 결정 사항

- **언어 설정은 기존 문서에 통합:** `RoutineConfig` 에 `language` 필드
  (`serde(default)` → En). 별도 설정 파일 없이 routines.json 하나가 앱 설정의
  source of truth 유지. 기존 문서 호환은 default 로 보장 (테스트 고정).
- **트레이도 함께 번역:** menu-switcher 때 만든 "상태 → 메뉴 전량 재구성"
  구조 덕에 언어 변경 → `notify_routines_changed` → 메뉴 재빌드로 즉시 반영.
  Rust 쪽 문자열은 `tray_text(Language)` 매치 테이블.
- **프런트는 무의존성 타입드 딕셔너리:** `src/shared/i18n/` 에 `messages.ts`
  (en 기준 `MessageKey` 파생, ko 는 `Record<MessageKey, string>` 라 누락 시
  컴파일 에러) + `LanguageContext` (`useT`/`useLanguage`). react-i18next 는
  이 규모(키 ~50개)에 과함.
- **검증 메시지는 키 반환:** `checkIsDraftValid` 가 문자열 대신 `MessageKey`
  를 반환해 표시 시점에 번역. region/preset 라벨은 템플릿 리터럴 키
  (`region.${region}`) 로 타입 안전하게 조회.
- **전환 UI:** 사이드바 하단 EN/한국어 토글. 실행 로그 시각도 언어별 로케일
  (`en-US`/`ko-KR`) 적용.

### 진행 단계

1. `dev` → `feat/i18n` 분기.
2. Rust: `Language` enum + 필드 + `set_language` (store/커맨드) + 트레이
   문자열 테이블. 테스트 3개 추가 (기본값 호환, lowercase serde).
3. 프런트: shared/i18n 모듈 + 전 컴포넌트 문자열 치환 + 사이드바 토글.
4. 테스트 51개 / fmt / clippy / pnpm build 통과.

### 다음 단계

- 백로그 동일. 언어 추가 시 `messages.ts` 딕셔너리 + `tray_text` 항목 추가로
  확장 (Language enum variant 추가).

## 2026-07-05 (feat/ui-redesign — UI/UX 전면 개편)

### 목적

사용자 피드백 3라운드에 걸친 UI/UX 재작업: (1) "어두컴컴한 프로토타입" →
테마 시스템, (2) "다크/라이트 색만 다름, 구조가 그대로" → 인터랙션 재설계,
(3) "AI 가 만든 티가 남" → 네이티브 macOS 문법 + 카피 전면 정리.

### 결정 사항

- **모니터가 곧 편집기:** 목업을 히어로 크기(430px)로 승격하고 인터랙션
  표면으로 전환 — 액션 카드 선택 → 모니터 영역 클릭으로 배치/해제. 액션 행의
  영역 드롭다운 제거, 배치 배지(✕로 해제)로 대체. 전체 화면용 클릭 타깃은
  스탠드 아래 알약형.
- **테마 시스템:** `Theme { System, Light, Dark }` 를 config 에 영속화,
  `data-theme` 속성 + CSS 변수 팔레트 2벌. 시스템 모드는 matchMedia 로 OS
  외형 실시간 추적.
- **"AI 티" 제거 원칙 (3차 피드백):** 그라디언트 텍스트/버튼·배경 글로우·장식
  서체 전부 제거 → 시스템 폰트(SF Pro), 플랫 배경, 헤어라인 보더, 단색 액센트
  (활성 상태에만), macOS 스타일 세그먼트(회색 트랙+흰 썸). 마이크로카피도
  문장형·대시(—) 제거하고 명사형 단문으로 ("실행 기록 없음" 등). 사이드바
  태그라인 삭제.
- **최초 실행 온보딩:** `language` 를 `Option<Language>` 로 바꿔 "미선택" 을
  모델에 정식화. None 이면 한국어/English 선택 화면(이중 언어 표기)부터.
  기존 문서(필드 없음)도 None 으로 로드돼 업그레이드 사용자도 한 번 지나감.
- **설정 화면 신설:** 사이드바 EN/한국어 토글 제거 → ⚙ 설정 뷰: 화면(테마) /
  언어 / 로그인 자동 실행(토글, 트레이와 동기화) / 권한(마이크 테스트·손쉬운
  사용) / 버전. 자동실행·마이크용 커맨드 3종 추가.
- **삭제 UX:** 사이드바 항목 hover ✕ + 편집기 삭제 버튼 모두 2단계 확인
  (`useArmedConfirm`: 첫 클릭 arm → 3초 내 재클릭 확정, 자동 해제). 저장/삭제
  푸터는 슬림 sticky 로 항상 노출 (스크롤에 묻히던 문제 해결).
- **앱 이름 자동완성:** `list_installed_apps` 커맨드가 /Applications 계열
  3곳을 스캔, `<datalist>` 로 타이핑 제안. 자유 입력 유지.
- **트레이 정리:** 무반응이던 "마이크 테스트" 항목 제거 (설정 탭의 피드백
  있는 테스트로 대체). 대신 캡처 실패 시 `run_on_main_thread` 로 상태 라벨을
  "마이크 권한 없음" 으로 갱신.
- **README:** 로드맵/Before-After/이모지 표 등 제거, Install(Releases 링크 +
  Gatekeeper 안내)/Usage/How it works/Build/Docs 구조로 건조하게 재작성.

### 진행 단계

1. `dev` → `feat/ui-redesign` 분기.
2. Rust: Theme + Option<Language> + set_theme/get·set_autostart/
   test_microphone/list_installed_apps 커맨드, 트레이 정리. 테스트 52개.
3. 프런트: shared/theme(useAppliedTheme) + shared/useArmedConfirm +
   SettingsView + 온보딩 + 편집기 재작성 + CSS 3차 개편.
4. 매 라운드 dev 앱 라이브로 사용자 피드백 → 반영 반복.

### 발견 사항

- rustfmt/clippy 급의 "정답" 이 없는 디자인 작업은 사용자 스크린샷 피드백
  루프가 필수였음. "AI 티" 의 실체는 그라디언트 텍스트·글로우·과잉 라운딩·
  문장형 마이크로카피·대시 남용으로 구체화됨.
- 스티키 푸터는 반드시 슬림하게 + 아래 콘텐츠 여백 확보. 첫 구현은 콘텐츠를
  가려 "삭제 버튼이 없다" 는 리포트로 이어짐.
...
## 2026-07-05 (fix/placement-polish — v0.1.0 편입)

### 변경

- `Region::Centered`(원래 크기) 추가: 창 크기를 유지한 채 대상 디스플레이
  중앙으로 이동만 수행. 모니터 목업 아래 전용 클릭/드롭 타깃 추가.
- 손쉬운 사용 권한 배너를 캔버스 하단에서 탭 바 아래 상단으로 이동
  (푸터 겹침 제거). 미승인 상태에서 3초 간격 + 창 포커스 시 재확인해
  시스템 설정에서 승인 후 복귀하면 즉시 반영. 설정 화면 권한 행 동일 적용.
